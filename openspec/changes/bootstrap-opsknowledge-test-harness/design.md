...
Create a Python 3.12 development-only gate for `test-harness`. `uv.lock` is the resolution authority; local and remote validation share ordered `make ci`, which first requires exactly `uv 0.11.29`. GitHub Actions supplies and verifies the same deterministic prerequisite, then delegates its only project-validation command to `make ci`. No runtime, production dependency, provider, database, or web surface is created.

## Architecture Decisions

| Decision | Alternatives / trade-off | Choice and rationale |
|---|---|---|
| Environment binding | Frozen resolution does not pin the `uv` executable | Before every local `make ci` gate, require `uv --version` exactly `uv 0.11.29`; then use `uv sync --frozen --extra dev` and `uv run --frozen`. A mismatch prints `ERROR: uv version mismatch; expected 0.11.29, found <actual>.` followed by `Remediation: install uv 0.11.29 and rerun make ci.`, then stops before sync. |
| CI bootstrap | Ambient/latest uv is nondeterministic | After existing checkout, use `astral-sh/setup-uv@d4b2f3b6ecc6e67c4457f6d3e41ec42d3d0fcb86 # v5.4.2`, `version: "0.11.29"`, then assert the same exact version before `make ci`, which repeats the local assertion. GitHub's authoritative Astral tag resolves to this verified commit; apply re-verifies it. |
...
| CI recipe | Local version parity, ordering/bootstrap | Stub `uv --version` for success, mismatch, and unavailable cases; assert exact remediation and no later sentinel on failure. Static workflow test checks setup SHA/comment, selected version, assertion-before-`make ci`, and existing security boundary. |
